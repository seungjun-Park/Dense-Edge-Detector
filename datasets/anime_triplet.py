...
class AnimeTripletDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        edge_0 = cv2.imread(f'{path}/edges_0/{name}', cv2.IMREAD_GRAYSCALE)
        edge_1 = cv2.imread(f'{path}/edges_1/{name}', cv2.IMREAD_GRAYSCALE)
        edge_2 = cv2.imread(f'{path}/edges_2/{name}', cv2.IMREAD_GRAYSCALE)

        img = self.to_tensor(img)
        edge_0 = self.to_tensor(edge_0)
        edge_1 = self.to_tensor(edge_1)
        edge_2 = self.to_tensor(edge_2)
        ...
        edge_2 = tf.resized_crop(edge_2, i, j, h, w, size=self.size, antialias=True)
        ...
        if random.random() < 0.5:
            ...
            edge_2 = self.horizontal_flip(edge_2)

        return img, edge_0, edge_1, edge_2
